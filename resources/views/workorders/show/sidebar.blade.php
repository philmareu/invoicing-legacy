<div class="uk-button-group uk-margin-bottom">
	...
	@if(! is_null($timer) && $timer->work_order_id == $workOrder->id)
        <button type="button" class="uk-button toggle-time uk-text-danger uk-width-1-5" id="{{ $workOrder->id }}">
            <i class="uk-icon-stop"></i>
            <span class="timer">{{ $timer->elapsedFormatted() }}</span>
        </button>
    @else
        <button type="button" class="uk-button toggle-time uk-text-success uk-width-1-5" id="{{ $workOrder->id }}">
